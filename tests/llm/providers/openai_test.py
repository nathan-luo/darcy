...
from llmgine.llm.providers.openai_provider import OpenAIProvider, OpenAIResponse
...
async def test_parallel_tool_calls(openai_provider):
    ...
    response = await provider.generate(
        context=[
            {
                "role": "user",
                "content": "What's the weather and time in New York and London?",
            }
        ],
        tools=tools,
...
async def test_response_format_json(openai_provider):
    ...
    response = await openai_provider.generate(
        context=[
            {
                "role": "user",
                "content": "Give me the data for the first 3 planets in JSON format",
            }
        ],
