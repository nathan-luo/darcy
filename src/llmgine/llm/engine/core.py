...
class LLMEngine:
    ...
    def _register_command_handlers(self) -> None:
        ...
        self.message_bus.register_command_handler(PromptCommand, self._handle_prompt)
    ...
    async def _handle_system_prompt(self, command: SystemPromptCommand) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(command_id=command.id, success=False, error=error_msg)
    ...
    async def _handle_clear_history(self, command: ClearHistoryCommand) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                command_id=command.id, success=True, result="Conversation history cleared"
            )
    ...
    async def _handle_prompt(self, command: PromptCommand) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(command_id=command.id, success=True, result=llm_response)
    ...
    async def _process_tool_calls(
        ...
        for tool_call in tool_calls:
            ...
            await self.message_bus.publish(
                ToolCallEvent(
                    tool_call.function.name, tool_call.function.arguments, conversation_id
                )
            )
